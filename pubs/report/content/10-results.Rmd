...
Together, those figure offer interesting insights into the direct and indirect effects of environmental drivers on the species of the Scotian Shelf. The drivers with the most intense direct and indirect effects across all taxa are shipping, demersal destructive high-bycatch fisheries (*e.g.* trawls and dredges), and negative sea-bottom temperature anomalies; these are essentially some of the most widely distributed drivers that were considered in the assessment. We can also see that all drivers affect all taxa indirectly, yet the same is not true for direct effects. For instance, the direct effects of shipping are mostly limited to marine mammals and a few species of fish. The same is true for certain taxa groups, with sea birds and marine mammals presenting the most stark contrasts between direct and indirect effects. A few sea bird species are exclusively affected by fisheries when considering direct effects. Similarly, marine mammals are affected solely by marine traffic and coastal drivers through direct pathways of effect. However, all sea bird and marine mammal species are affected by all driver types when indirect effects are taken into consideration. Interestingly, we also show that the overall intensity of indirect effects on vertebrates tends to be greater than that of direct effects.
...
```{r meta2, out.width = "90%", echo=FALSE, fig.align='center', fig.link=urls$meta2, fig.cap=cap$meta2}
...
```{r contr, out.width = "90%", echo=FALSE, fig.align='center', fig.link=urls$cont, fig.cap=cap$cont}
knitr::include_graphics(urls$cont)
```

<br/>

```{r contrsp, out.width = "90%", echo=FALSE, fig.align='center', fig.link=urls$contsp, fig.cap=cap$contsp}
