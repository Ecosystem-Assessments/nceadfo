...
fig_contribution_group <- function(cekm, period) {
  ...
  dL <- c(yG, 1.03) # This is so periods have same axes (not reproducible)
  # iL <- c(-yG, min(ind$ymax)-yG) # This is the reproducible code
  iL <- c(-yG, -.58) # This is so periods have same axes (not reproducible)
  # Polygons
  ...
  for(i in 1:nrow(spGroup)) {
    text(x = i, y = yMin+.25, labels = spGroup$gr2[i], adj = c(1,.5), srt = 90, cex = 1)
  }
