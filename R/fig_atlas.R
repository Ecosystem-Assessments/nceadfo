...
fig_atlas <- function(type = c("aoi", "footprint", "cea", "difference", "metanetwork", "contribution", "species", "drivers")) {
  ...
  if ("cea" %in% type) {
    ...
    # -----------------
    # Species-scale cea
    i1 <- magick::image_read(here::here(out$figs, "cea_2010_2015.png"))
    i2 <- magick::image_read(here::here(out$figs, "cea_2016_2021.png"))
    img <- magick::image_append(c(i1, i2))
    ...
    # -----------------
    # Network-scale cea
    i1 <- magick::image_read(here::here(out$figs, "ncea_2010_2015.png"))
    i2 <- magick::image_read(here::here(out$figs, "ncea_2016_2021.png"))
    img <- magick::image_append(c(i1, i2))
  ...
  if ("metanetwork" %in% type) {
    ...
    # -----------------
    i1 <- magick::image_read(here::here(out$figs, "metanetwork-Direct_2016_2021.png"))
    i2 <- magick::image_read(here::here(out$figs, "metanetwork-Indirect_2016_2021.png"))
  ...
  if ("species" %in% type) {
    ...
    # Filter on species for which there is an assessment only
    filter_names <- function(dat) {
      nm_ass <- basename(net1) |>
      ...
      uid <- tmp$species %in% nm_ass$species
      dat[uid]
    }
    cnt <- filter_names(cnt)
    bin <- filter_names(bin)
    sp1 <- filter_names(sp1)
    sp2 <- filter_names(sp2)
    net1 <- filter_names(net1)
    net2 <- filter_names(net2)
